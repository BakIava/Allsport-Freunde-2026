...
export default function EventForm({ event }: EventFormProps) {
  ...
  const [formData, setFormData] = useState<EventCreateInput>({
    ...
    max_per_email: (event as { max_per_email?: number } & typeof event)?.max_per_email ?? 5,
  });
        ...
        <CardContent>
          <form id="event-form" onSubmit={(e) => handleSubmit(e, false)} className="space-y-6 pb-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="max_per_email">Max. Personen pro E-Mail-Adresse</Label>
                <Input
                  id="max_per_email"
                  type="number"
                  min={1}
                ...
                />
                <p className="text-xs text-muted-foreground">Wie viele Personen kann eine E-Mail-Adresse insgesamt anmelden (Standard: 5)</p>
              </div>
            </div>
